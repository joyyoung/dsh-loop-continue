 ...
 * claims a configuration card for that namespace onto the Plugins page's
 * bundle entry (`plugins.bundle.config`), so the judge policy and the steering
 * text are editable without touching `settings.yaml` by hand.
 *
...
`
     ...
     * Client plugin body.
     *
     * On DSH 0.1.7 the `settingsScope` service was replaced by `configForms`,
     * and the configurable-tab slot `settings.plugin.item` moved to the bundle
     * configuration slot `plugins.bundle.config` on the Plugins page. The form
     * handle is obtained once (configForms.get) and handed to the card through
     * the registration's `inject` face; `whileServed` mounts the entry only
     * while the Host serves the namespace, preserving the old guard where the
     * bind and the slot registration rode one scope.
     * @param ctx - client root context.
     */
    function apply(ctx) {
      ctx.effect(() => installStyles(), `${PLUGIN_ID}: card styles`)

      // The card renders on the Plugins page's bundle detail only while the
      // Host serves its namespace, mirroring the pre-0.1.7 binding guard.
      ...
      }), `${PLUGIN_ID}: bundle config`)
    }

    return { apply, name: PLUGIN_ID, inject: ['slots', 'configForms'] }
  },
})
